...
def df_to_parquet(date):
    ...
    buffer = BytesIO()
    with buffer:
        with pq.ParquetWriter(buffer, table.schema) as writer:
...
#    dataframe.write.parquet(f'file://{local_path}')
    ...
    # S3 업로드
    s3 = create_s3client()
    s3.put_object(Bucket='sms-basket', Key=f'{parquet_path}/{filename}.parquet', Body=parquet_data)
...
# df.show()
# S3에서 Parquet 파일을 읽어 BytesIO 객체에 저장
s3 = create_s3client()
s3_object = s3.get_object(Bucket='sms-basket', Key='spark/kobis/2023/20230904_boxOffice.parquet')
buffer = BytesIO(s3_object['Body'].read())

# PyArrow를 사용하여 BytesIO 객체를 Table로 읽기
table = pq.read_table(buffer)

# Convert to list of dictionaries (one dictionary per row)
data = [{col: table[col][i].as_py() for col in table.column_names} for i in range(len(table))]
